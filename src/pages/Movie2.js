...
componentDidMount() {  
    ...
    axios.get('https://api.themoviedb.org/3/search/movie?api_key=5f6907c45589a114e83c963b14160d13&query=avatar&language=en-US&page=1&include_adult=false')
      .then(response => {
        this.setState({title: response.data.results[2].title});
		this.setState({overview: response.data.results[2].overview});
		this.setState({imageurl: response.data.results[2].poster_path});
      })	  
